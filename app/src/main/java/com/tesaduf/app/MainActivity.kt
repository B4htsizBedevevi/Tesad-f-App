...
class MainActivity : Activity() {
    ...
    private fun showProfileSetup() {
        val root=baseRoot();val scroll=ScrollView(this).apply{isFillViewport=true};val content=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(dp(24),dp(18),dp(24),dp(28))};val logo=AnimatedLogoView(this);content.addView(logo,lp(-1,dp(145),1,0));content.addView(label("Önce seni tanıyalım",28f,white,Typeface.BOLD));content.addView(label("Kayıt yok. İsim, telefon veya e-posta istemiyoruz. Sana sadece anonim bir kimlik veriyoruz.",15f,muted),lp(-1,-2,1,12));val idCard=roundedCard();idCard.addView(label("ANONİM KİMLİĞİN",11f,muted,Typeface.BOLD));val id=getOrCreateAnonymousId();idCard.addView(label(id,30f,white,Typeface.BOLD),lp(-1,-2,1,6));idCard.addView(label("Bu kodla sohbetlerde görünürsün.",13f,muted),lp(-1,-2,1,2));content.addView(idCard,lp(-1,-2,1,20));content.addView(label("Avatarını seç",18f,white,Typeface.BOLD),lp(-1,-2,1,4));content.addView(label("İstersen daha sonra değiştirebilirsin.",13f,muted),lp(-1,-2,1,10));val avatars=listOf("🌙","⚡","🎧","🐺","🦊","🌌","🎮","🪐");val avatarRow=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.CENTER};var selected=prefs.getString("avatar",avatars[0])?:avatars[0];val avatarViews=mutableListOf<TextView>();avatars.forEach{avatar->val v=avatarButton(avatar,avatar==selected);v.tag=avatar;v.setOnClickListener{selected=avatar;avatarViews.forEach{updateAvatarState(it,it.tag==selected)}};avatarViews.add(v);avatarRow.addView(v,lp(0,dp(58),1,5))};content.addView(avatarRow,lp(-1,-2,1,18));val continueButton=primaryButton("TESADÜFE BAŞLA");continueButton.setOnClickListener{prefs.edit().putString("anonymous_id",id).putString("avatar",selected).putString("profile_ready","yes").apply();showHome()};content.addView(continueButton,lp(-1,dp(54),1,16));content.addView(label("Kimlik bilgilerin cihazında anonim olarak saklanır.",12f,muted).apply{gravity=Gravity.CENTER},lp(-1,-2,1,12));scroll.addView(content);root.addView(scroll,lp(-1,-1,1,0));setContentView(root)
    }

    private fun showHome(){val root=baseRoot();val content=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(dp(22),dp(16),dp(22),dp(18))};val logo=AnimatedLogoView(this);content.addView(logo,lp(-1,dp(100),1,0));val top=LinearLayout(this).apply{gravity=Gravity.CENTER_VERTICAL};top.addView(label(prefs.getString("avatar","🌙")?:"🌙",28f,white),lp(dp(48),dp(48),0,0));val identity=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL};identity.addView(label("TESADÜF",20f,white,Typeface.BOLD));identity.addView(label(prefs.getString("anonymous_id","#????????")?:"#????????",13f,muted),lp(-1,-2,1,2));top.addView(identity,lp(0,-2,1,12));content.addView(top);content.addView(label("Bugün kimin hikâyesine denk geleceksin?",28f,white,Typeface.BOLD),lp(-1,-2,1,30));val hero=roundedCard();hero.gravity=Gravity.CENTER;hero.addView(label("✦",46f,accent2,Typeface.BOLD).apply{gravity=Gravity.CENTER});hero.addView(label("Bir tesadüf başlat.",21f,white,Typeface.BOLD).apply{gravity=Gravity.CENTER},lp(-1,-2,1,8));hero.addView(label("Rastgele biriyle anonim bir sohbet.\n15 dakika. Gerisini tesadüfe bırak.",14f,muted).apply{gravity=Gravity.CENTER},lp(-1,-2,1,4));val start=primaryButton("TESADÜFÜ BAŞLAT");start.setOnClickListener{startActivity(android.content.Intent(this,MatchActivity::class.java))};hero.addView(start,lp(-1,dp(52),1,20));content.addView(hero,lp(-1,0,1,20));val privacy=roundedCard();privacy.addView(label("ANONİMLİK ÖNCELİĞİ",11f,accent2,Typeface.BOLD));privacy.addView(label("Gerçek ad yok • Profil fotoğrafı yok • Telefon yok",14f,white),lp(-1,-2,1,7));privacy.addView(label("Sadece sohbet. Sadece o an.",13f,muted),lp(-1,-2,1,2));content.addView(privacy,lp(-1,-2,1,0));content.addView(label("Ayarlar   •   Engellenenler   •   Güvenlik",12f,muted).apply{gravity=Gravity.CENTER},lp(-1,-2,1,18));root.addView(content,lp(-1,-1,1,0));setContentView(root)}
